...
export default LaunchScreen = props => {
    ...
    const selectCart = useCallback(async(id, name, image, distance, width, length) => {
        try {
            await dispatch(CartsActions.SelectCart(id, name, image, distance, width, length ))
        } catch (error) {
            console.log(error)
        }
    })
          ...
          <View style={styles.modal}>
            <View style={{position: "absolute", right: 20, bottom: 20}}>
                <TouchableOpacity onPress={() => {
                    ...
                    selectCart(selectedValue.id, selectedValue.name, selectedValue.imageUri, selectedValue.distance, selectedValue.width, selectedValue.length)
                    props.navigation.navigate("Main", {
                        screen: selectedValue.name == "Add your cart" ? "Carts" : "Home", 
                    })
            ...
            <SelectDropdown
                ...
                defaultButtonText="Choose a cart"
                
            />
            {errorMessage ? <Text style={{color: "red", fontWeight: "bold"}}>You have to pick a cart</Text> : <Text></Text>}
